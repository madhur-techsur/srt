...
- Success acknowledgment displayed (delegated to F03)
- Inline error message displayed on API failure
- Table refresh triggered on successful submission (delegated to F02)
...
9. **On HTTP 201 response:**
   ...
   b. Clear the Success/Error acknowledgment area, then display a success message (F03).
   c. Reset all form fields to empty.
   d. Re-enable the Submit button.
   e. Trigger a refresh of the Request List View (F02) — either via shared state or a new GET call.
...
- Success acknowledgment message after HTTP 201 response (see F03 for display spec).
...
- Trigger for Request List View refresh (see F02).
...
- Client-side validation is a UX convenience; server-side validation (F01) is the authoritative gate.
